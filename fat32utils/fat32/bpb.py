...
class Fat32BPB:
  def __init__(self, boot_sector):
    self.bpb = boot_sector[0xB:0x60]

  def bytes_per_sector(self):
    return int.from_bytes(self.bpb[:0x2], fat32.LE)

  def sectors_per_cluster(self):
    return int(self.bpb[2])

  def reserved_sectors(self):
    return int.from_bytes(self.bpb[0x3:0x5], fat32.LE)

  def number_of_fats(self):
    return int(self.bpb[0x5])

  def total_sectors(self):
    return int.from_bytes(self.bpb[0x15:0x19], fat32.LE)
  
  def sectors_per_fat(self):
    return int.from_bytes(self.bpb[0x19:0x1d], fat32.LE)

  def root_dir_cluster(self):
    return int.from_bytes(self.bpb[0x21:0x25], fat32.LE)
